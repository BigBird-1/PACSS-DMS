...
from interfaceTest.constants import wfs_urls
...
class ToDo(object):
    ...
    def query(self, order_no):
        ...
        for i in range(30):
            res = http_r.run_main('post', url=wfs_urls["代办事项查询"], data=payload, name="代办事项列表查询")
    ...
    def audit_node(self):
        ...
        payload = json.dumps(payload)
        res = http_r.run_main('post', url=wfs_urls["获取审核信息"], data=payload, name="获取审核节点信息")
    ...
    def from_data(self):
        ...
        if self.module_name in ["车辆采购入库审核", "固定资产", "单车综合毛利标准审核", "特殊订单审核"]:
            ...
            value_row = eval(table[0]["sub_attr_ex"][0]['attr_ex_value_row'])
            order_info = [{} for i in range(len(value_row))]  # 子表单
            row_num = table[0]['row_num']
            for i in range(row_num):
                value_row = eval(table[0]["sub_attr_ex"][i]['attr_ex_value_row'])
                for j in range(len(value_row)):
                    order_info[j][table[0]["sub_attr_ex"][i]['attr_module_name']] = value_row[j]
    ...
    def can_end(self):
        ...
        payload = json.dumps(payload)
        res = http_r.run_main('post', url=wfs_urls["是否可终结"], data=payload, name="当前审核节点是否可终结")
        log.info(res)
    ...
    def next_auditor(self, audit_code):
        ...
        payload = json.dumps(payload)
        res = http_r.run_main('post', url=wfs_urls["下一级审批人员列表"], data=payload, name="下一级审批人员列表查询")
        next_info = None
    ...
    def audit(self, audit_result, audit_content, next_audit_code, next_code="", next_user="", end=None):
        ...
        payload = json.dumps(payload)
        res = http_r.run_main('post', url=wfs_urls["审核通过/驳回"], data=payload, name="审核结果-通过/驳回")
