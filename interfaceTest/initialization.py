...
from interfaceTest.constants import globals_urls
...
class Initialization(object):
    ...
    def vehicle_main(product_code=""):
        product_params = {
            "searchData": {"productCodeKey": "true", "isValid": "12781001", "productCode": product_code}
        }
    ...
    @staticmethod
    def customer_info(phone=""):
        params = {"limit": 30, "offset": 0, "searchData": {"keyWord": phone, "funCode": "001"}}
        res = http_r.run_main('get', url=globals_urls["选择客户列表查询"], data=params, name="选择客户列表查询")
        if res["data"]["total"] == 0:
            log.info("选择客户列表查询-未查到数据")
            return
        content = random.choice(res["data"]["list"])  # 从30个里随机选择一个
...
if __name__ == '__main__':
    # client_info = initial.customer_info()
    # print(client_info)
