...
from apiTest.constants import sales_urls
...
class SalesOrder(object):
    ...
    def new_save(self, vin, phone="", customer_type=""):
        if options["允许进行配车"] == 12781002:
            log.error("没有配车权限")
            return
        # --------------------------------------------------------------------------------------------------------------
        ct_no = randomData.random_card()  # 随机生成身份证号码
        client_info = initial.customer_info(phone, customer_type)  # 客户详细信息
        ...
        if vin_info["dispatchedStatus"] in ["已配车确认", "已交车", "已交车确认"]:
            ...
            return
        # actual_price = int(vin_info["factoryVehiclePrice"]*0.96)  # 销售订单实际售价
        actual_price = vin_info["factoryVehiclePrice"]
        # --------------------------------------------------------------------------------------------------------------
        data = {
        ...
        if ct_code is None:
            ...
            if ct_nu == "":
                if ct_code == 12391001:
                    ...
                    ct_nu = ct_no[:8]
        # --------------------------------------------------------------------------------------------------------------
        sold_dict = self.init_data["soldByList"][0]
        sales_order_detail = {
            "soStatus": 13011010,
            ...
            "cashSaleDiscount": vin_info["factoryVehiclePrice"]-actual_price,  # 折让合计
            "newCarDiscount": vin_info["factoryVehiclePrice"]-actual_price,  # 新车折让
            ...
            "innerColor": vin_info["inner_color_code"],
            "innerColorDesc": vin_info["innerColor"],
            "colorCode": vin_info["outer_color_code"],
            "colorCodeDesc": vin_info["outerColor"],

            "optionalCode": vin_info["optional_code"],
            "optionalPrice": vin_info["optionalPrice"],
            "optionalName": vin_info["optionalName"],
            "outerColorCode": vin_info["outer_color_code"],
            "outerColorName": vin_info["outerColor"],
            "innerColorName": vin_info["innerColor"],
            "innerColorCode": vin_info["inner_color_code"],

            "totalDirectivePrice": vin_info["factoryVehiclePrice"],

            # "fixedAssetsType": vin_info["fixedAssetsType"],
    ...
    def submit_audit(so_no):
        ...
        log.info("{}----{}".format(res, res["message"]))
...
if __name__ == '__main__':
    ...
    # sales_order.submit_audit("SN2206160004")
    sales_order.new_save(vin="HAUR3ZKF16Y0XP279", phone="15845826236")
