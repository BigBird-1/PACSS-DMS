...
from apiTest.constants import deposit_urls
...
class DepositOrder(object):
    ...
    def new_save(self, code_desc, phone=""):
        ...
        # -----------------------动力类型-------------------------------------------------------------------------------
        params = {"searchData": {"productCode": product_data["productCode"]}}
        res = http_r.run_main('get', url=deposit_urls["动力类型查询"], data=params, name="动力类型查询")
        ...
        # ---------------------------------------------------------------------------------------------------------------
        params = {"productCode": product_data["productCode"]}
        res = http_r.run_main('get', url=deposit_urls["获取产品内/外饰色"], data=params, name="获取产品内/外饰色")
        ...
        # ---------------------------------------------------------------------------------------------------------------
        params = {"searchData": {"productCode": product_data["productCode"]}}
        res = http_r.run_main('get', url=deposit_urls["获取选装包"], data=params, name="获取选装包")
        ...
        order_data = {
            ...
            "outerColorCode": outer_color[0]["outerColorCode"],
            "outerColorName": outer_color[0]["outerColorName"],
            "innerColorCode": inner_color[0]["innerColorCode"],
            "innerColorName": inner_color[0]["innerColorName"],
            ...
            "optionalPrice": optional_price,
            "totalDirectivePrice": product_data["directivePrice"] + optional_price,
            ...
            "useCarAddress": " 仙女座河外星系M31",  # 用车地址
            "powerType": power_type,  # 动力类型
